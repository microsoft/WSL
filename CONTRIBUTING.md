...
WSL's localized resource files (`localization/strings/<locale>/Resources.resw`) are generated by a separate localization team and are regenerated downstream, so any translation edits made in a pull request here would be overwritten and cannot be merged. Filing an issue lets us route the fix to the team that owns those translations. The English source strings (`localization/strings/en-US/Resources.resw`) are the exception, since those originate in this repository.
