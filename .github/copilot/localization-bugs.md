...
translation, or by opening a **pull request** that edits `localization/strings/<locale>/Resources.resw`
(or adds a brand-new locale file). Both are valid signals and you should look for **both** when triaging.

Either way, the fix is handled by the Microsoft Global Collaboration Service (GCS) localization pipeline,
**not** merged directly from GitHub: every non-`en-US` `Resources.resw` is generated by the GCS team and
regenerated downstream, so any translation edit merged here would be overwritten. (PRs that touch
localized files should be declined with a pointer to `CONTRIBUTING.md`; only `en-US` source strings live
in this repo.) To route a report for review, file a tracking **Bug** in the GCS Azure DevOps project for
each issue or PR.
...
Scope: localization/strings/<locale>/Resources.resw (for a PR: <edits to existing | brand-new>, +A/-B, 1 file;
  for an issue: the specific string(s) reported).
