...
static std::wstring ResolveWslExecutablePath()
{
    ...
    // exercise the current build, while falling back to system32 wsl.exe in
    // environments that only have the packaged binary.
    ...
    return LxssGenerateWslCommandLine(nullptr, L"wsl.exe");
}

static wil::unique_handle StartSleepingWslProcess(int seconds, DWORD createFlags = CREATE_UNICODE_ENVIRONMENT | EXTENDED_STARTUPINFO_PRESENT)
{
    const std::wstring arguments = std::format(L"-- sh -c \"sleep {}\"", seconds);
    const std::wstring wslPath = ResolveWslExecutablePath();
    std::wstring commandLine = std::format(L"\"{}\" {}", wslPath, arguments);
    return LxsstuStartProcess(commandLine.data(), nullptr, nullptr, nullptr, nullptr, createFlags);
}
...
static bool WaitForInputModeChange(HANDLE conin, DWORD originalMode, DWORD timeoutMs, DWORD& observedMode)
{
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(timeoutMs);
    do
    {
        if (GetConsoleMode(conin, &observedMode) && (observedMode != originalMode))
        {
            return true;
        }
...
class WSLCCLIVTSupportUnitTests
{
    ...
    TEST_METHOD(ConsoleState_ConcurrentInputClients_OutOfOrderRestore)
    {
        wil::unique_hfile conin{CreateFileW(
            L"CONIN$", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr)};
        if (!conin)
        ...
        auto first = std::make_unique<wsl::windows::common::ConsoleState>();
        first->SetInteractiveMode();

        DWORD configured{};
        VERIFY_WIN32_BOOL_SUCCEEDED(GetConsoleMode(conin.get(), &configured));

        auto second = std::make_unique<wsl::windows::common::ConsoleState>();
        second->SetInteractiveMode();

        DWORD duringBoth{};
        VERIFY_WIN32_BOOL_SUCCEEDED(GetConsoleMode(conin.get(), &duringBoth));
        VERIFY_ARE_EQUAL(configured, duringBoth);
    ...
    TEST_METHOD(ConsoleState_ConcurrentInputClients_ExternalModeChangePreserved)
    {
        wil::unique_hfile conin{CreateFileW(
            L"CONIN$", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr)};
        if (!conin)
        {
            LogSkipped("Skipping ConsoleState external-change test: CONIN$ is not available (no attached console)");
            return;
        }
        ...
        DWORD externallyAdjusted = configured;
        bool changed = false;
        const std::array<DWORD, 4> candidates = {
            configured | ENABLE_PROCESSED_INPUT,
            configured | ENABLE_LINE_INPUT,
            configured ^ ENABLE_INSERT_MODE,
            configured ^ ENABLE_WINDOW_INPUT,
        };

        for (const DWORD candidate : candidates)
        {
            if ((candidate != configured) && SetConsoleMode(conin.get(), candidate))
            {
                externallyAdjusted = candidate;
                changed = true;
                break;
            }
        }
        ...
        if (!changed)
        {
            LogSkipped("Skipping ConsoleState external-change test: could not apply an alternate valid input mode");
            return;
        }
        ...
        DWORD afterFirstExit{};
        VERIFY_WIN32_BOOL_SUCCEEDED(GetConsoleMode(conin.get(), &afterFirstExit));
        VERIFY_ARE_EQUAL(externallyAdjusted, afterFirstExit);

        second.reset();

        DWORD finalMode{};
        VERIFY_WIN32_BOOL_SUCCEEDED(GetConsoleMode(conin.get(), &finalMode));
        VERIFY_ARE_EQUAL(externallyAdjusted, finalMode);
    }
    ...
    TEST_METHOD(ConsoleState_WslProcesses_SharedConsole_OutOfOrderRestore)
    {
        ...
        auto processA = StartSleepingWslProcess(3);
        VERIFY_IS_TRUE(!!processA);

        DWORD configured{};
        VERIFY_IS_TRUE(WaitForInputModeChange(conin.get(), baseline, 5000, configured));

        auto processB = StartSleepingWslProcess(7);
        VERIFY_IS_TRUE(!!processB);
    ...
    TEST_METHOD(ConsoleState_WslProcesses_SeparateConsoles_Isolation)
    {
        ...
        auto process = StartSleepingWslProcess(2, CREATE_NEW_CONSOLE | CREATE_UNICODE_ENVIRONMENT | EXTENDED_STARTUPINFO_PRESENT);
    ...
    TEST_METHOD(ConsoleState_WslProcesses_SharedConsole_CrashPreservesRemainingClient)
    {
        wil::unique_hfile conin{CreateFileW(
            L"CONIN$", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr)};
        if (!conin)
        {
            LogSkipped("Skipping ConsoleState crash test: CONIN$ is not available (no attached console)");
            return;
        }
        ...
        auto processA = StartSleepingWslProcess(10);
        VERIFY_IS_TRUE(!!processA);

        DWORD configured{};
        VERIFY_IS_TRUE(WaitForInputModeChange(conin.get(), baseline, 5000, configured));

        auto processB = StartSleepingWslProcess(12);
        VERIFY_IS_TRUE(!!processB);

        VERIFY_WIN32_BOOL_SUCCEEDED(TerminateProcess(processA.get(), 0xDEAD));
        (void)WaitForProcessExit(processA.get(), 5000);

        DWORD afterCrash{};
        VERIFY_WIN32_BOOL_SUCCEEDED(GetConsoleMode(conin.get(), &afterCrash));
        VERIFY_ARE_EQUAL(
            configured,
            afterCrash,
            L"Crashing one shared-console client must not restore baseline while another client remains alive");
