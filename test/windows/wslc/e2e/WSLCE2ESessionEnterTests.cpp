...
class WSLCE2ESessionEnterTests
{
    ...
    WSLC_TEST_METHOD(WSLCE2E_SessionList_Verbose_PrintsSummary)
    {
        constexpr auto sessionName = L"test-wslc-session-list-verbose";

        auto session = RunWslcInteractive(std::format(L"system session enter \"{}\" --name {}", GetDefaultStoragePath(), sessionName));
        VERIFY_IS_TRUE(session.IsRunning(), L"Session should be running");
        session.ExpectStdout(VT::SESSION_PROMPT);
        auto cleanup = wil::scope_exit([&]() { session.Exit(); });

        // --verbose prepends a "[wslc] Found N session(s)" summary line that plain list does not.
        ...
        VERIFY_IS_TRUE(verboseResult.Stdout->find(sessionName) != std::wstring::npos);
        ...
        VERIFY_IS_TRUE(plainResult.Stdout->find(L"[wslc] Found ") == std::wstring::npos);
    }

    WSLC_TEST_METHOD(WSLCE2E_SessionShell_TargetByName)
    {
        constexpr auto sessionName = L"test-wslc-session-shell";
        ...
        // `system session shell <name>` attaches a new shell to the named session (positional arg).
        auto shellSession = RunWslcInteractive(std::format(L"system session shell {}", sessionName));
        VERIFY_IS_TRUE(shellSession.IsRunning(), L"Shell session should be running");
        shellSession.ExpectStdout(VT::SESSION_PROMPT);

        shellSession.WriteLine("echo wslc-shell-target");
        shellSession.ExpectStdout(VT::RESET);
        shellSession.ExpectCommandEcho("echo wslc-shell-target");
        shellSession.ExpectStdout("wslc-shell-target\r\n");
        shellSession.ExpectStdout(VT::SESSION_PROMPT);

        VERIFY_ARE_EQUAL(0, shellSession.Exit());

        // Exiting the sub-shell must leave the original session alive.
        auto listResult = RunWslc(L"system session list");
    ...
    WSLC_TEST_METHOD(WSLCE2E_SessionTerminate_TargetByName)
    {
        constexpr auto sessionName = L"test-wslc-session-terminate";

        auto session = RunWslcInteractive(std::format(L"system session enter \"{}\" --name {}", GetDefaultStoragePath(), sessionName));
        VERIFY_IS_TRUE(session.IsRunning(), L"Session should be running");
        session.ExpectStdout(VT::SESSION_PROMPT);
        // After terminate the entered shell's connection is gone; force-kill rather than try
        // to send an `exit` command that would never reach a live session.
        auto cleanup = wil::scope_exit([&]() {
            if (session.IsRunning())
            {
                session.Terminate();
        ...
        auto terminateResult = RunWslc(std::format(L"system session terminate {}", sessionName));
        terminateResult.Verify({.Stderr = L"", .ExitCode = 0});
