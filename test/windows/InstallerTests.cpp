...
class InstallerTests
{
    ...
    TEST_METHOD(InitrdImgLifecycle)
    {
        ...
        // 2. Be removed and regenerated on upgrade (via WiX RemoveFile + custom action)
        // 3. Be removed on uninstall
        ...
        // Install 2.0.2 which ships initrd.img in the MSI
        InstallGitHubRelease(L"2.0.2");
        VERIFY_IS_TRUE(IsMsiPackageInstalled());
        VERIFY_IS_TRUE(std::filesystem::exists(initrdPath), L"initrd.img should exist after installing 2.0.2");

        // Upgrade to current version and verify initrd.img exists (regenerated by custom action)
        InstallMsi();
        VERIFY_IS_TRUE(IsMsiPackageInstalled());
        VERIFY_IS_TRUE(std::filesystem::exists(initrdPath), L"initrd.img should exist after upgrade (generated by installer)");

        // Launch WSL to verify it works
        ValidatePackageInstalledProperly();

        // Verify initrd.img still exists
        VERIFY_IS_TRUE(std::filesystem::exists(initrdPath), L"initrd.img should still exist after launching WSL");

        // Uninstall and verify initrd.img is removed
        UninstallMsi();
        VERIFY_IS_FALSE(IsMsiPackageInstalled());
