...
#define CONFIG_FILE_PATH L"/etc/sysctl.d/MirroredLoopbackSystemd.conf"
    ...
    WSL2_TEST_METHOD(AcceptedConnectionKeepsPortReservedGuestClient)
    {
        MIRRORED_NETWORKING_TEST_ONLY();
        ...
        // Start a guest process that listens, accepts a single connection, closes the listen socket,
        // then keeps the accepted connection open by echoing data back (via cat).
        // socat TCP-LISTEN closes the listener after accepting one connection, which is exactly the
        // scenario being tested: the port tracker must not release the port while the accepted
        // connection is still alive.
        auto [stdErrRead, stdErrWrite] = CreateSubprocessPipe(false, true);
        auto [stdOutRead, stdOutWrite] = CreateSubprocessPipe(false, true);

        const std::wstring wslCmd = L"socat -dd TCP4-LISTEN:1234,reuseaddr EXEC:cat";
        auto cmd = LxssGenerateWslCommandLine(wslCmd.data());
        auto guestProcess = unique_kill_process(LxsstuStartProcess(cmd.data(), nullptr, stdOutWrite.get(), stdErrWrite.get()));
        stdErrWrite.reset();
        stdOutWrite.reset();

        // Wait for socat to start listening
        std::string output;
        VERIFY_IS_TRUE(NetworkTests::FindSubstring(stdErrRead, "listening on", output));

        // Connect from a guest client that keeps the connection alive
        auto [clientStdErrRead, clientStdErrWrite] = CreateSubprocessPipe(false, true);
        const std::wstring clientCmd = L"socat -dd TCP4-CONNECT:127.0.0.1:1234 EXEC:'sleep 60'";
        auto clientCmdLine = LxssGenerateWslCommandLine(clientCmd.data());
        auto clientProcess = unique_kill_process(LxsstuStartProcess(clientCmdLine.data(), nullptr, nullptr, clientStdErrWrite.get()));
        clientStdErrWrite.reset();

        // Wait for the server to accept the connection (it closes the listen socket after accepting)
        VERIFY_IS_TRUE(NetworkTests::FindSubstring(stdErrRead, "starting data transfer loop", output));

        // Verify the listen socket is actually closed — no LISTEN state socket on port 1234
        VERIFY_ARE_EQUAL(LxsstuLaunchWsl(L"ss -tln sport = :1234 | grep -q LISTEN"), static_cast<DWORD>(1));

        // The listen socket is now closed but the accepted connection is still alive.
        // Verify the port tracker has NOT released the port — host should not be able to bind it.
    ...
    WSL2_TEST_METHOD(AcceptedConnectionKeepsPortReservedHostClient)
    {
        MIRRORED_NETWORKING_TEST_ONLY();
        ...
        const wil::unique_socket clientSocket(socket(AF_INET, SOCK_STREAM, IPPROTO_TCP));
        VERIFY_ARE_NOT_EQUAL(clientSocket.get(), INVALID_SOCKET);
        ...
        wsl::shared::retry::RetryWithTimeout<void>(connectPred, std::chrono::seconds(1), std::chrono::minutes(2));

        // Wait for socat to accept the connection (it closes the listen socket after accepting)
        VERIFY_IS_TRUE(NetworkTests::FindSubstring(stdErrRead, "starting data transfer loop", output));

        // Verify the listen socket is actually closed — no LISTEN state socket on port 1234
        VERIFY_ARE_EQUAL(LxsstuLaunchWsl(L"ss -tln sport = :1234 | grep -q LISTEN"), static_cast<DWORD>(1));

        // Send data and verify it echoes back through the accepted connection
        const char sendBuf[] = "hello";
        VERIFY_ARE_NOT_EQUAL(send(clientSocket.get(), sendBuf, sizeof(sendBuf), 0), SOCKET_ERROR);

        char recvBuf[64] = {};
        int Timeout = 5000;
        VERIFY_ARE_NOT_EQUAL(setsockopt(clientSocket.get(), SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<char*>(&Timeout), sizeof(Timeout)), SOCKET_ERROR);
        const auto bytesReceived = recv(clientSocket.get(), recvBuf, sizeof(recvBuf), 0);
        VERIFY_IS_TRUE(bytesReceived > 0);
        VERIFY_ARE_EQUAL(std::string_view(recvBuf, bytesReceived), std::string_view(sendBuf, sizeof(sendBuf)));
