...
static void VerifyDnsSuffixes()
{
    ...
    if (!foundSuffix)
    {
        ...
        auto restoreGlobalSuffixes =
            wil::scope_exit([&] { LxsstuLaunchPowershellAndCaptureOutput(L"Set-DnsClientGlobalSetting -SuffixSearchList @()"); });
...
static std::wstring CreateSocatString(const SOCKADDR_INET& si, int protocol, bool listen)
{
    return std::wstring(((protocol == IPPROTO_TCP) ? L"TCP" : L"UDP")) + std::wstring(((si.si_family == AF_INET) ? L"4" : L"6")) +
           ...
           std::wstring(
               (listen)
                   ? std::to_wstring(ntohs(SS_PORT(&si))) + std::wstring(L",bind=") + wsl::windows::common::string::SockAddrInetToWstring(si)
                   : wsl::windows::common::string::SockAddrInetToWstring(si) + std::wstring(L":") + std::to_wstring(ntohs(SS_PORT(&si))));
}
...
struct GuestListener
{
    GuestListener(const SOCKADDR_INET& addr, int protocol)
    {
        ...
        std::string output;
        THROW_HR_IF(E_FAIL, !FindSubstring(readPipe, "listening on", output));
    }
    ...
    {
        std::string output;
        VERIFY_IS_TRUE(FindSubstring(readPipe, "starting data transfer loop", output));
    }
...

    {
        ...
        auto pred = [&]() {
            if (protocol == IPPROTO_UDP)
            {
                const char buffer = 'A';
                THROW_HR_IF(
                    E_FAIL, sendto(clientSocket.get(), &buffer, sizeof(buffer), 0, reinterpret_cast<SOCKADDR*>(&addr), sizeof(addr)) != sizeof(buffer));
            }
            else
            {
...
static void FirewallSettingEnabledTests(bool isHyperVFirewallEnabled)
{
    ...
    // Add host block rule, which is expected to be enforced
    blockRule.Type = FirewallType::Host;
    AddFirewallRuleAndValidateTraffic(blockRule, isHyperVFirewallEnabled ? FirewallTestConnectivity::Blocked : FirewallTestConnectivity::Allowed);
    // Add hyper-v block rule, which is expected to be enforced
    blockRule.Type = FirewallType::HyperV;
    AddFirewallRuleAndValidateTraffic(blockRule, isHyperVFirewallEnabled ? FirewallTestConnectivity::Blocked : FirewallTestConnectivity::Allowed);
...
class NatTests
{
    ...
    */
    ...
    TEST_METHOD(WithoutIcsDnsProxy)
    {
        WSL2_TEST_ONLY();
    ...
    TEST_METHOD(LocalhostRelayNoIpv6)
    {
        WSL2_TEST_ONLY();
    ...
    TEST_METHOD(FirewallRulesExpectedBlockFirewallDisabled)
    {
        HYPERV_FIREWALL_TEST_ONLY();
        SKIP_TEST_UNSTABLE();
    ...
    TEST_METHOD(FirewallRulesExpectedAllowFirewallDisabled)
    {
        HYPERV_FIREWALL_TEST_ONLY();
        SKIP_TEST_UNSTABLE();
    ...
    TEST_METHOD(FirewallRulesEnabledSettingFirewallDisabled)
    {
        HYPERV_FIREWALL_TEST_ONLY();
        SKIP_TEST_UNSTABLE();
    ...
    TEST_METHOD(FirewallRulesExpectedBlockFirewallDisabledByPolicy)
    {
        HYPERV_FIREWALL_TEST_ONLY();

        RegistryKeyChange<DWORD> policyChange(
...
#define CONFIG_FILE_PATH L"/etc/sysctl.d/MirroredLoopbackSystemd.conf"
    ...
    TEST_METHOD(HostUdpBindDoesNotPreventGuestTcpBind)
    {
        ...
        auto tcpPort = BindHostPort(12345, SOCK_STREAM, IPPROTO_TCP, true);
        auto udpPort = BindGuestPort(L"UDP4-LISTEN:12345", true);
    }
