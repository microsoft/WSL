...

{
    ...
    if (MayReclaim && Cache >= 0)
    {
        ...
        if (Excess > c_gradualHysteresisBytes)
        {
            ...
            // Best-effort: RequestCgroupReclaim never throws and does not log the expected EAGAIN, so a
            // transient write error cannot tear down the long-lived reduction thread.
            Reclaimed = RequestCgroupReclaim(ToFree);
