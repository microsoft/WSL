...
ExecStart=/bin/mount -o bind,ro,X-mount.mkdir -t none /mnt/wslg/.X11-unix /tmp/.X11-unix)";
        ...
        if (interopEnabled && protectBinfmt)
        {
            ...
            // Re-assert WSL's binfmt handler after systemd-binfmt's ExecStart. The first two echoes
            // tolerate failures (e.g. concurrent registration from another distro) and the final
            // greps verify the live entry has the correct interpreter and the 'F' (fix-binary) flag.
            const auto execStartPostCommand = std::format(
                R"(/bin/sh -c '(echo -1 > {0}/{1} 2>/dev/null || true) ; (echo "{2}" > {3} 2>/dev/null || true) ; )"
                R"(grep -q "^interpreter /init$" {0}/{1} && grep -q "^flags:.*F" {0}/{1}')",
