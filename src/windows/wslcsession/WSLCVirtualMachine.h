...
private:
    ...
    // Queries the guest kernel for the hv_pci swiotlb pool that was reserved at boot
    // and forwards the values to the service so that subsequent virtio device-options
    // can include the swiotlb token. Called after the root filesystem is mounted.
    void ReadGuestSwiotlbConfig();
