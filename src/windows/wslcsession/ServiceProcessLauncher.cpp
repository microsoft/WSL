...
using wsl::windows::service::wslc::ServiceRunningProcess;
using wsl::windows::service::wslc::WSLCProcess;

ServiceRunningProcess::ServiceRunningProcess(const Microsoft::WRL::ComPtr<WSLCProcess>& process, WSLCProcessFlags flags) :
    common::RunningWSLCProcess(flags)
{
...
WSLCProcess& ServiceRunningProcess::Get()
{
    return *m_process.Get();
}

void ServiceRunningProcess::GetState(WSLCProcessState* State, int* Code)
{
    THROW_IF_FAILED(m_process->GetState(State, Code));
}

std::tuple<HRESULT, int, std::optional<ServiceRunningProcess>> ServiceProcessLauncher::LaunchNoThrow(WSLCVirtualMachine& virtualMachine)
{
...
ServiceRunningProcess ServiceProcessLauncher::Launch(WSLCVirtualMachine& virtualMachine)
{
