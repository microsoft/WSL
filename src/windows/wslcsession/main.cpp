...
Abstract:
    ...
    Entry point for wslcsession.exe - the per-user COM server for WSLC sessions.

    This runs under the user's identity and hosts WSLCSessionFactory COM objects.
    The SYSTEM service creates sessions via IWSLCSessionFactory::CreateSession,
...
class WSLCSessionFactoryClassFactory : public winrt::implements<WSLCSessionFactoryClassFactory, IClassFactory>
{
public:
    ...
    try
    {
        *result = nullptr;
        ...
        auto factory = Microsoft::WRL::Make<wsl::windows::service::wslc::WSLCSessionFactory>();
