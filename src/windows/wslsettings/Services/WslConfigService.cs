...
public class WslConfigService : IWslConfigService
{
    ...
    // Dirty tracking: stores only entries the user has changed in-app but not yet committed.
    ...
    public uint SetWslConfigSetting(IWslConfigSetting wslConfigSetting)
    {
        ...
        lock (_wslCoreConfigInterfaceLockObj!)
        {
            ...
            try
            {
                bool isDirty = !persisted.Equals(settingManaged.GetValueAsObject());
    ...
    public uint CommitPendingChanges()
    {
        ...
        lock (_wslCoreConfigInterfaceLockObj!)
        {
            ...
            try
            {
                foreach (var (entry, value) in _pendingValues)
                {
                    // Build a native setting, apply the pending value, and write it
