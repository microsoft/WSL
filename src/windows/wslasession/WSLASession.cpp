...
try
{
    ...
    // Accumulate lines and use nlohmann::json::accept() to detect complete JSON objects.
    // This handles both compact (single-line) and pretty-printed (multi-line) rawjson output.
    auto captureOutput = [&](const gsl::span<char>& content) {
        ...
        auto json = nlohmann::json::parse(pendingJson, nullptr, false);
        pendingJson.clear();
