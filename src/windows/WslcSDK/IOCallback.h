...
struct IOCallback
{
    IOCallback(IWSLCSDKProcess* process, const WslcContainerProcessIOCallbackOptions& options);
    ~IOCallback();
    ...
    static wil::unique_handle GetIOHandle(IWSLCSDKProcess* process, WslcProcessIOHandle ioHandle);

private:
    wil::com_ptr<IWSLCSDKProcess> m_process;
