...
void PluginManager::RegisterWslPump(ULONG SessionId, const std::shared_ptr<PluginCallPump>& Pump)
{
...
HRESULT PluginManager::InvokeOnWslPump(ULONG SessionId, std::function<HRESULT()> Work)
{
    ...
    for (;;)
    {
        ...
        std::shared_ptr<PluginCallPump> pump;
        {
...
HRESULT PluginManager::RunHostNotification(OutOfProcPlugin& Plugin, ULONG SessionId, const std::function<HRESULT(IWslPluginHost*)>& Notify)
{
    auto pump = std::make_shared<PluginCallPump>();
...
void PluginManager::OnVmStopping(const WSLSessionInformation* Session)
{
    ...
    for (auto& e : m_plugins)
    {
        ...
        const HRESULT result = RunHostNotification(e, Session->SessionId, [&](IWslPluginHost* host) {
            return host->OnVMStopping(Session->SessionId, Session->UserToken, static_cast<DWORD>(sidData.size()), sidData.data());
        });
