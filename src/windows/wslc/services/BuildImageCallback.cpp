...
try
{
    ...
    {
        m_reporter.Info(L"{}", MultiByteToWide(status));
        return S_OK;
    }
    ...
    // them when --no-color is set. Color() additionally strips them in plain mode. The trailing
    // newlines are emitted after the reset.
    m_reporter.Info(L"{}{}{}{}", Color(Format::Fg::BrightGreen), wide, Color(Format::Default), newlines);
    return S_OK;
}
CATCH_RETURN();
...
void BuildImageCallback::Redraw()
{
    ...
    // under --no-color and Color() strips in plain mode. The buffered body carries the cursor
    // moves, erases, and text lines.
    m_reporter.Info(L"{}{}{}{}{}", Cursor::Hide, Color(Format::Dim), std::wstring_view{m_frameBuffer}, Color(Format::Normal), Cursor::Show);
