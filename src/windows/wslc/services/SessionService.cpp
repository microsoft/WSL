...
int SessionService::Enter(const std::wstring& storagePath, const std::wstring& displayName)
{
    ...
    // Build session settings from user config, overriding storage path and display name.
    WSLCSessionSettings settings{};
    settings.DisplayName = displayName.c_str();
    settings.StoragePath = storagePath.c_str();
    settings.CpuCount = settings::User().Get<settings::Setting::SessionCpuCount>();
    settings.MemoryMb = settings::User().Get<settings::Setting::SessionMemoryMb>();
    settings.BootTimeoutMs = 30 * 1000;
    settings.MaximumStorageSizeMb = settings::User().Get<settings::Setting::SessionStorageSizeMb>();
    settings.NetworkingMode = settings::User().Get<settings::Setting::SessionNetworkingMode>();
    ...
    // Ensure the session is terminated when we leave, regardless of how we exit.
    auto terminateOnExit = wil::scope_exit([&] { LOG_IF_FAILED(session->Terminate()); });
    ...
    // Launch with terminal fds (PTY).
    wsl::windows::common::WSLCProcessLauncher launcher{
        shell, {shell, "--login"}, {"TERM=xterm-256color"}, WSLCProcessFlagsTty | WSLCProcessFlagsStdin};
    launcher.SetTtySize(windowSize.Y, windowSize.X);
    auto process = launcher.Launch(*session);
    auto tty = process.GetStdHandle(WSLCFDTty);
    auto updateTerminalSize = [&]() {
    ...
    wslutil::PrintMessage(Localization::MessageWslcShellExited(string::MultiByteToWide(shell), static_cast<int>(exitCode)), stdout);
