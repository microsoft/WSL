...
public:
    ...
    BuildImageCallback(Reporter& reporter, HANDLE cancelEvent, bool verbose, models::ProgressMode mode = models::ProgressMode::Tty) :
        m_reporter(reporter), m_verbose(verbose), m_cancelEvent(cancelEvent), m_mode(mode), m_color(mode == models::ProgressMode::Tty)
    {
    }
    ~BuildImageCallback();
...
private:
    ...
    // the Reporter emits nothing for it. Used to strip color in plain mode while keeping cursor moves.
