...
public:
    ...
    BuildImageCallback(Reporter& output, HANDLE cancelEvent, bool verbose) :
        m_output(output), m_verbose(verbose), m_cancelEvent(cancelEvent)
    {
    }
    ~BuildImageCallback();
...
private:
    ...
    // as a single atomic WriteConsoleW/fwprintf and respects the global output state.
    ...
    bool m_isConsole = m_output.IsVTEnabled(Reporter::Level::Info);
